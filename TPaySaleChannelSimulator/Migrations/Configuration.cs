namespace TPaySaleChannelSimulator.Migrations
{
    ...
    internal sealed class Configuration : DbMigrationsConfiguration<TPaySaleChannelSimulator.Models.TPayDb>
    {
        ...
        protected override void Seed(TPaySaleChannelSimulator.Models.TPayDb context)
        {
            ...
            context.saleChannel.AddOrUpdate(new SaleChannel{operatorId= O1.Id,merchantId=M1.Id});
            context.saleChannel.AddOrUpdate(new SaleChannel { operatorId = O1.Id, merchantId = M2.Id });
            context.saleChannel.AddOrUpdate(new SaleChannel { operatorId = O2.Id, merchantId = M3.Id });
            context.saleChannel.AddOrUpdate(new SaleChannel { operatorId = O4.Id, merchantId = M3.Id });
            context.saleChannel.AddOrUpdate(new SaleChannel { operatorId = O2.Id, merchantId = M4.Id });
            context.saleChannel.AddOrUpdate(new SaleChannel { operatorId = O4.Id, merchantId = M4.Id });
        }
